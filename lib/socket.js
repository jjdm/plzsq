...
const WebSocketManager = function () {
	...
	let handleMessage = function(json) {
		...
		if(!match) {
			throw new Error(`Could not match for message type ${message.type}.`);
		}
	};
	...
	self.initialize = function(wss) {
		...
		_wss.on('connection', function connection(ws) {
			let parts = ws.upgradeReq.url.split('='); // e.g. /?user=a1
			let userId = parts[1];
			log.debug("User %s connected via Websocket: %s", userId, Object.keys(ws));
			ws.user = userId;
